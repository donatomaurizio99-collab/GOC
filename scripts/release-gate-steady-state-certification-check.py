...
def run_check(
    *,
    ...
    post_cutover_finalization_report_file: Path,
    ...
    required_signals_raw = policy.get("required_certification_signals")
    ...
    if not required_signals:
        policy_errors.append("required_certification_signals_missing")
    ...
    post_cutover_payload = (
        _read_json_object(post_cutover_finalization_report_file)
        if post_cutover_finalization_report_file.exists()
        else {}
    )
    ...
    post_cutover_action = str((post_cutover_payload.get("decision") or {}).get("recommended_action") or "")
    ...
    post_cutover_signal_ok = post_cutover_action in {"production_ready", "production_ready_finalized"}
    ...
    signal_status = {
        "post_release_watch": bool(post_release_watch_signal_ok),
        "post_cutover_finalization": bool(post_cutover_signal_ok),
    ...
    if unknown_required_signals:
        policy_errors.append("required_certification_signals_unknown")
    ...
    criteria = [
        ...
        _criterion(
            "steady_state_policy_valid",
            len(policy_errors) == 0,
            f"policy_errors={len(policy_errors)}",
        ),
        _criterion(
            "steady_state_required_signal_coverage",
        ...
        _criterion(
            "steady_state_green_report_quorum",
        ...
        _criterion(
            "steady_state_post_release_watch_signal",
            bool(post_release_watch_signal_ok),
            f"recommended_action={post_release_watch_action!r}",
        ),
        _criterion(
            "steady_state_post_cutover_signal",
            bool(post_cutover_signal_ok),
            f"recommended_action={post_cutover_action!r}",
        ),
        _criterion(
            "steady_state_burnin_threshold",
    ...
    report = {
        ...
        "paths": {
            ...
            "post_cutover_finalization_report_file": str(post_cutover_finalization_report_file),
        ...
        "policy": {
            ...
            "required_certification_signals": required_signals,
        ...
        "metrics": {
            ...
            "steady_state_reports_non_green": len(non_green_reports),
            "label_mismatch_reports": len(label_mismatch_reports),
            "steady_state_release_block_signals": len(release_block_signal_reports),
            "steady_state_policy_invalid": len(policy_errors),
            "steady_state_required_signals_total": len(required_signals),
            "steady_state_required_signals_failed": len(failed_required_signals) + len(unknown_required_signals),
            "steady_state_green_reports_count": int(green_reports_count),
            "steady_state_required_green_reports_failed": 0 if required_green_reports_met else 1,
            "steady_state_watch_signal_failed": 0 if post_release_watch_signal_ok else 1,
            "steady_state_post_cutover_signal_failed": 0 if post_cutover_signal_ok else 1,
            "steady_state_burnin_threshold_failed": 0 if burnin_threshold_ok else 1,
            "steady_state_closure_signal_failed": 0 if closure_signal_ok else 1,
            "criteria_failed": len(failed_criteria),
        },
        "decision": {
            "release_blocked": not bool(success),
            "recommended_action": "block_release" if not success else "production_ready_steady_state",
        },
        "criteria": criteria,
    ...
    if not success and not allow_not_ready:
        raise RuntimeError(f"Release-gate steady-state certification check failed: {json.dumps(report, sort_keys=True)}")
    return report
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Validate steady-state production certification from Stage-AG post-release watch "
            "plus post-cutover/P0 stability evidence."
        )
    )
    parser.add_argument("--label", default="release-gate-steady-state-certification-check")
    parser.add_argument("--project-root")
    parser.add_argument("--policy-file", default="docs/release-gate-steady-state-certification-policy.json")
    ...
    parser.add_argument("--output-file", default="artifacts/release-gate-steady-state-certification-release-gate.json")
    ...
    post_cutover_finalization_report_file = _resolve_path(project_root, args.post_cutover_finalization_report_file)
    ...
    try:
        report = run_check(
            ...
            post_cutover_finalization_report_file=post_cutover_finalization_report_file,
        ...
        print(f"[release-gate-steady-state-certification-check] ERROR: {exc}", file=sys.stderr)
        return 1
