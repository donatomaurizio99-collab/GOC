    ...
    *,
    ...
    report = {
        ...
        "metrics": {
            ...
            "release_train_reports_non_green": len(non_green_reports),
            "label_mismatch_reports": len(label_mismatch_reports),
            "release_train_block_signals": len(release_block_signal_reports),
            "criteria_failed": len(failed_criteria),
        },
        "decision": {
            "release_blocked": not bool(success),
            "recommended_action": "block_release" if not success else "proceed_to_stage_ab",
        },
        "criteria": criteria,
    ...
    if not success and not allow_not_ready:
        raise RuntimeError(f"Release-gate release-train readiness check failed: {json.dumps(report, sort_keys=True)}")
    return report
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Consolidate Stage-T/U/V/W/X/Y/Z signals into one release-train readiness verdict."
        )
    )
    parser.add_argument("--label", default="release-gate-release-train-readiness-check")
    ...
    parser.add_argument("--output-file", default="artifacts/release-gate-release-train-readiness-release-gate.json")
    ...
    try:
        ...
        print(f"[release-gate-release-train-readiness-check] ERROR: {exc}", file=sys.stderr)
        return 1
