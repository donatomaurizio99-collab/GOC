...
def run_check(
    *,
    ...
    reports_with_release_block_signal: list[dict[str, Any]] = []
    ...
    for report_path in resolved_required_reports:
        ...
        if has_release_block_signal:
            reports_with_release_block_signal.append(record)

    burnin_payload = loaded_payload_by_path.get(str(burnin_report_file), {})
    if not burnin_payload and burnin_report_file.exists():
    ...
    burnin_required_consecutive = _coerce_int(burnin_metrics.get("required_consecutive"), required_consecutive)
    ...
    criteria = [
        ...
        _criterion(
            ...
            len(reports_with_release_block_signal) == 0,
            f"reports_with_release_block_signal={len(reports_with_release_block_signal)}",
        ),
        _criterion(
    ...
    report = {
        ...
        "metrics": {
            ...
            "reports_with_release_block_signal": len(reports_with_release_block_signal),
            "burnin_consecutive_green": int(burnin_consecutive_green),
            "burnin_required_consecutive": int(burnin_required_consecutive),
        ...
        "reports_with_release_block_signal": reports_with_release_block_signal,
    ...
    if not success and not allow_not_ready:
        raise RuntimeError(f"Release-gate production readiness certification failed: {json.dumps(report, sort_keys=True)}")
    return report
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Generate the final production-readiness go/no-go certificate from Stage-P/Q/R/S plus P0 burn-in evidence."
        )
    )
    parser.add_argument("--label", default="release-gate-production-readiness-certification")
    ...
    parser.add_argument(
        "--output-file",
        default="artifacts/release-gate-production-readiness-certification-release-gate.json",
    )
    ...
    try:
        ...
        print(f"[release-gate-production-readiness-certification] ERROR: {exc}", file=sys.stderr)
        return 1
