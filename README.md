...
```
Registry attestation gate command (attests sync report + lock payload invariants):
```powershell
.\scripts\run-release-gate-registry-attestation-gate.ps1 -RegistrySyncReportFile artifacts\release-gate-registry-sync-ci.json -OutputFile artifacts\release-gate-registry-attestation-gate-ci.json
```
Write/sync command:
```powershell
...
It also enforces strict-flag parity between registry `release_gate_ci.strict_flags` and `[switch]$Strict*` declarations in `scripts\release-gate.ps1`.
...
Nightly drift/warning workflows now upsert deduplicated GitHub issues (labels: `ci-drift` + signal label) so alerts become explicit action items.
...
```powershell
Set-Location "C:\Users\raffa\OneDrive\Documents\New project"
.\scripts\run-ci-alert-issue-upsert.ps1 -SignalId "release-gate-runtime-early-warning" -ReportFile "artifacts\release-gate-runtime-early-warning.json" -DryRun
```
...
- [release-gate-registry-sync.py](/C:/Users/raffa/OneDrive/Documents/New%20project/scripts/release-gate-registry-sync.py)
- [run-release-gate-registry-sync.ps1](/C:/Users/raffa/OneDrive/Documents/New%20project/scripts/run-release-gate-registry-sync.ps1)
- [release-gate-registry-attestation-gate.py](/C:/Users/raffa/OneDrive/Documents/New%20project/scripts/release-gate-registry-attestation-gate.py)
- [run-release-gate-registry-attestation-gate.ps1](/C:/Users/raffa/OneDrive/Documents/New%20project/scripts/run-release-gate-registry-attestation-gate.ps1)
