...
def test_154_ci_release_artifact_includes_stage_d_runtime_evidence_reports():
    ...
    registry_attestation_script = (
        project_root / "scripts" / "release-gate-registry-attestation-gate.py"
    ).read_text(encoding="utf-8")
    registry_attestation_wrapper = (
        project_root / "scripts" / "run-release-gate-registry-attestation-gate.ps1"
    ).read_text(encoding="utf-8")
    ...
    assert "Attest release-gate registry sync report" in ci_workflow
    assert (
        '.\\scripts\\run-release-gate-registry-attestation-gate.ps1 -RegistrySyncReportFile '
        '"artifacts/release-gate-registry-sync-ci.json" -OutputFile '
        '"artifacts/release-gate-registry-attestation-gate-ci.json"'
    ) in ci_workflow
    ...
    assert (
        ...
        in registry["p0_runbook_contract"]["required_runbook_scripts"]
    )
    assert "artifacts/release-gate-registry-sync-ci.json" in registry["p0_runbook_contract"]["required_ci_artifact_paths"]
    assert (
        "artifacts/release-gate-registry-attestation-gate-ci.json"
        in registry["p0_runbook_contract"]["required_ci_artifact_paths"]
    )
    ...
    assert "run-release-gate-registry-sync.ps1" in runbook_contract_script
    assert "run-release-gate-registry-attestation-gate.ps1" in runbook_contract_script
    ...
    assert "artifacts/release-gate-registry-sync-ci.json" in runbook_contract_script
    assert "artifacts/release-gate-registry-attestation-gate-ci.json" in runbook_contract_script
    ...
    assert "metrics.post_release_watch_reports_non_green=0" in runbook_contract_script
    assert "metrics.post_release_watch_release_block_signals=0" in runbook_contract_script
    assert "metrics.post_release_watch_non_ok_window_violations=0" in runbook_contract_script
    assert "metrics.post_release_watch_chaos_regression_violations=0" in runbook_contract_script
    assert "metrics.post_release_watch_finalization_signal_failed=0" in runbook_contract_script
    assert "metrics.steady_state_reports_non_green=0" in runbook_contract_script
    assert "metrics.steady_state_release_block_signals=0" in runbook_contract_script
    ...
    assert "release-gate-registry-sync.py" in registry_attestation_script
    assert 'parser.add_argument("--registry-sync-report-file", default=' in registry_attestation_script
    assert 'parser.add_argument("--expected-registry-sync-report-path", default=' in registry_attestation_script
    assert "release-gate-registry-attestation-gate.py" in registry_attestation_wrapper
    assert '[string]$RegistrySyncReportFile = "artifacts\\\\release-gate-registry-sync-ci.json"' in registry_attestation_wrapper
    assert '[string]$OutputFile = ""' in registry_attestation_wrapper
    assert '"--registry-sync-report-file", $RegistrySyncReportFile' in registry_attestation_wrapper
    assert '@("--output-file", $OutputFile)' in registry_attestation_wrapper
...
def test_197_release_gate_post_release_watch_check_fails_when_non_ok_window_budget_is_exceeded():
    workspace = _local_test_dir("pytest-release-gate-post-release-watch-failure").resolve()
    project_root = Path(__file__).resolve().parents[1]
    artifacts_dir = workspace / "artifacts"
    artifacts_dir.mkdir(parents=True, exist_ok=True)

    post_cutover_report = artifacts_dir / "release-gate-post-cutover-finalization-release-gate.json"
    burn_rate_report = artifacts_dir / "release-gate-slo-burn-rate-v2-release-gate.json"
    chaos_report = artifacts_dir / "release-gate-chaos-matrix-continuous-release-gate.json"
    handoff_report = artifacts_dir / "release-gate-operations-handoff-readiness-release-gate.json"
    policy_file = workspace / "release-gate-post-release-watch-policy.json"
    output_file = artifacts_dir / "release-gate-post-release-watch-release-gate.json"

    post_cutover_report.write_text(
        json.dumps(
            {
                "label": "release-gate",
                "success": True,
        ...
        json.dumps(
            {
                "label": "release-gate",
                "success": True,
                "metrics": {"non_ok_window_violations": 2},
                "decision": {"release_blocked": False},
            },
            ensure_ascii=True,
            sort_keys=True,
    ...
    policy_file.write_text(
        json.dumps(
            {
                "version": "1.0.0",
                "required_watch_windows": ["h_plus_24", "h_plus_72", "h_plus_168"],
                "max_non_ok_window_violations": 0,
                "max_chaos_regression_violations": 0,
                "min_required_green_reports": 4,
                "runbook_section": "3.49 Post-release watch gate",
            },
            ensure_ascii=True,
            sort_keys=True,
        ),
    ...
    command = [
        sys.executable,
        str(project_root / "scripts" / "release-gate-post-release-watch-check.py"),
        "--label",
        "pytest-drill",
        "--project-root",
        str(workspace.resolve()),
        ...
        ",".join(
            str(path.resolve())
            for path in [post_cutover_report, burn_rate_report, chaos_report, handoff_report]
        ),
        "--finalization-report-file",
        str(post_cutover_report.resolve()),
        "--burn-rate-report-file",
        str(burn_rate_report.resolve()),
        "--chaos-report-file",
        str(chaos_report.resolve()),
        "--required-label",
        "release-gate",
        "--output-file",
        str(output_file.resolve()),
    ...
    assert completed.returncode != 0
    marker = "[release-gate-post-release-watch-check] ERROR: "
    assert marker in completed.stderr
    payload_text = completed.stderr.split(marker, 1)[1].strip()
    nested_marker = "Release-gate post-release watch check failed: "
    if payload_text.startswith(nested_marker):
        payload_text = payload_text.split(nested_marker, 1)[1]
    payload = json.loads(payload_text)
    assert payload["success"] is False
    assert payload["metrics"]["post_release_watch_non_ok_window_violations"] == 1
    assert payload["metrics"]["criteria_failed"] >= 1
    assert output_file.exists()
...
def test_198_release_gate_steady_state_certification_check_succeeds_for_green_chain():
    workspace = _local_test_dir("pytest-release-gate-steady-state-certification-success").resolve()
    project_root = Path(__file__).resolve().parents[1]
    artifacts_dir = workspace / "artifacts"
    artifacts_dir.mkdir(parents=True, exist_ok=True)
    ...
    closure_report = artifacts_dir / "p0-closure-report-release-gate.json"
    handoff_report = artifacts_dir / "release-gate-operations-handoff-readiness-release-gate.json"
    policy_file = workspace / "release-gate-steady-state-certification-policy.json"
    output_file = artifacts_dir / "release-gate-steady-state-certification-release-gate.json"
        ...
        json.dumps(
            {
                "label": "release-gate",
                "success": True,
                "metrics": {"consecutive_green": 12, "required_consecutive": 10},
                "decision": {"release_blocked": False},
            },
            ensure_ascii=True,
            sort_keys=True,
    ...
    policy_file.write_text(
        json.dumps(
            {
                "version": "1.0.0",
                "required_certification_signals": [
                    "post_release_watch",
                    "post_cutover_finalization",
                    "burnin_consecutive_green",
                    "p0_closure_go_no_go",
                ],
                "min_required_green_reports": 5,
                "runbook_section": "3.50 Steady-state certification",
            },
            ensure_ascii=True,
            sort_keys=True,
        ),
    ...
    command = [
        sys.executable,
        str(project_root / "scripts" / "release-gate-steady-state-certification-check.py"),
        "--label",
        "pytest-drill",
        "--project-root",
        str(workspace.resolve()),
        ...
        ",".join(
            str(path.resolve())
            for path in [post_release_watch_report, post_cutover_report, burnin_report, closure_report, handoff_report]
        ),
        "--post-release-watch-report-file",
        str(post_release_watch_report.resolve()),
        "--post-cutover-finalization-report-file",
        str(post_cutover_report.resolve()),
        "--burnin-report-file",
        str(burnin_report.resolve()),
        "--closure-report-file",
    ...
    payload = json.loads([line.strip() for line in completed.stdout.splitlines() if line.strip()][-1])
    assert payload["success"] is True
    assert payload["metrics"]["steady_state_reports_non_green"] == 0
    assert payload["metrics"]["steady_state_release_block_signals"] == 0
    assert payload["metrics"]["steady_state_watch_signal_failed"] == 0
...
def test_212_release_gate_registry_sync_fails_when_release_gate_strict_switch_has_no_runtime_usage():
    workspace = _local_test_dir("pytest-release-gate-registry-sync-strict-switch-no-runtime-usage").resolve()
    project_root = Path(__file__).resolve().parents[1]
    release_gate_file = workspace / "release-gate.ps1"
    release_gate_text = (project_root / "scripts" / "release-gate.ps1").read_text(encoding="utf-8")

    needle = "$StrictWorkflowWorkerRestartDrill"
    declaration_index = release_gate_text.find(needle)
    assert declaration_index >= 0
    release_gate_without_runtime_usage = (
        release_gate_text[: declaration_index + len(needle)]
        + release_gate_text[declaration_index + len(needle) :].replace(needle, "$WorkflowWorkerRestartDrill")
    )
    release_gate_file.write_text(release_gate_without_runtime_usage, encoding="utf-8")
...
def test_214_release_gate_registry_attestation_gate_reports_success():
    workspace = _local_test_dir("pytest-release-gate-registry-attestation-gate-success").resolve()
    project_root = Path(__file__).resolve().parents[1]
    artifacts_dir = workspace / "artifacts"
    artifacts_dir.mkdir(parents=True, exist_ok=True)
    sync_report_file = artifacts_dir / "release-gate-registry-sync-check.json"
    output_file = artifacts_dir / "release-gate-registry-attestation-check.json"
    ...
    assert sync_completed.returncode == 0, sync_completed.stderr
    assert sync_report_file.exists()

    command = [
        sys.executable,
        str(project_root / "scripts" / "release-gate-registry-attestation-gate.py"),
        "--project-root",
        str(project_root.resolve()),
        "--registry-sync-report-file",
        str(sync_report_file.resolve()),
        "--output-file",
        str(output_file.resolve()),
    ]
    ...
    assert payload["decision"]["release_blocked"] is False
    assert payload["decision"]["recommended_action"] == "registry_attestation_passed"
    assert output_file.exists()

    shutil.rmtree(workspace, ignore_errors=True)


def test_215_release_gate_registry_attestation_gate_fails_when_sync_report_declares_drift():
    workspace = _local_test_dir("pytest-release-gate-registry-attestation-gate-drift-failure").resolve()
    project_root = Path(__file__).resolve().parents[1]
    artifacts_dir = workspace / "artifacts"
    artifacts_dir.mkdir(parents=True, exist_ok=True)
    sync_report_file = artifacts_dir / "release-gate-registry-sync-check.json"
    ...
    sync_payload = json.loads(sync_report_file.read_text(encoding="utf-8"))
    sync_payload["changed"] = True
    sync_report_file.write_text(json.dumps(sync_payload, ensure_ascii=True, sort_keys=True), encoding="utf-8")
    ...
    assert completed.returncode != 0
    marker = "[release-gate-registry-attestation-gate] ERROR: "
    assert marker in completed.stderr
    payload_text = completed.stderr.split(marker, 1)[1].strip()
    nested_marker = "Release-gate registry attestation gate failed: "
    if payload_text.startswith(nested_marker):
        payload_text = payload_text.split(nested_marker, 1)[1]
    payload = json.loads(payload_text)
    assert payload["success"] is False
